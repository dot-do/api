...
router.get('/', (req) => {
  ...
  return {
    ...
    links: {
      'GET /posts - List all posts': `${base}/posts`,
      'GET /posts?published=true - Published only': `${base}/posts?published=true`,
      'GET /posts?published=false - Drafts only': `${base}/posts?published=false`,
      'GET /stats - Statistics': `${base}/stats`,
      'GET /version - PostgreSQL version': `${base}/version`,
      'POST /query - Raw SQL (body: {"sql": "..."})': `${base}/query`,
      'WSS /ws - WebSocket (95% cheaper)': `${wsBase}/ws`,
      'GET /ping - Health check (instant)': `${base}/ping`,
      'GET /debug - Lifecycle info (instant)': `${base}/debug`,
      'GET /debug/full - Full debug (waits for WASM)': `${base}/debug/full`,
    },
    timing: {

 ...
 *
 * Operation tags are the bare canonical operationIds (five-surface
 * invariant: route = MCP tool = suite ref = meter tag = rates key).
 *
 * The traffic seam is the §9.3 standing diagnostic's raw signal: identity
 * class (machine vs human) × referral source × brand — agent-referred HUMAN
 * traffic on this face flags the cell's missing human-vocabulary counterpart
 * brand (recorded in ../projection.json; the row rules consumer demand
 * agent-intermediated only).
 */

const SUBSTRATE = 'arts-entertainment'
const PROJECTION = 'arts-entertainment-placeholder' // GAP row — G4 brand pending (#16)
const MOTION = 'B2A'
const PATTERN = '402-metered' // experiment registration, ../projection.json

/** Identity class per §9.3: machine (id.org.ai grain when wired) vs human, + referral. */
 ...
 *  A money event with settled:true would be fake billing and is never emitted. */
...
export function emitReceipt(request, { operation }) {
  emit('receipt', { substrate: SUBSTRATE, projection: PROJECTION, operation, stub: true })
}

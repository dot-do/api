 ...
 * fabricated event — a fake money event is fake billing. When a real 402
 * re-authorization settles, the settlementRef here is the `ref` handed to
 * the rail ledger's POST /settlements door (LEDGER.md §4).
 */
 ...
 * brand. Agent-referred HUMAN traffic on this developer face = a missing
 * human-counterpart brand in the cell. The row's ICP includes non-technical
 * principals (credentialing coordinators, practice managers) and the estate
 * holds NO human-vocabulary credentialing name — the gap is recorded in
 * ../projections/healthcare.org.ai.json, and this seam is what triggers the
 * counterpart-brand proposal per spec §9.3.
 */

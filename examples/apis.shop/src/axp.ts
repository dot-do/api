 ...
 * estate extensions the generator does not carry yet, each at its RULED
 * placement (batch ruling: bridge exactly here until the upstream re-vendor
 * lands — never patched into the vendored files):
 *
 *   1. `rates[]` TOP-LEVEL on the served Pricing Document (DRAFT §2 extension;
 ...
 *   3. card `g2` member TOP-LEVEL — the row's G2 coordinates (ICP + Persona +
 *      System coordinates) per stake #6; the full document is `/icp.json`
 *      (card `links.icp`)
 *   4. `operationId` on every route in the OpenAPI doc (the rate card may
...
export const pricingDoc = (() => {
  ...
  return {
    ...base,
    rates: RATES,
    // offers mirrored here for one-document reads (DRAFT §2)
...
const pricingFaces = {
  json: pricingDoc,
  md: mdOfJson('apis.shop — pricing (rate card)', pricingDoc),
  html: htmlOfJson('apis.shop — pricing (rate card)', pricingDoc),
}

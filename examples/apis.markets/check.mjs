#!/usr/bin/env node
/**
 * check.mjs — the fail-closed wave-zero gate for apis.markets
 ...
 *   4. Fixture law: every seed record labeled (example: true; "[demo]" prefix
 *      on every name/title field; ZZ-prefixed synthetic symbols)
 *   5. Suite bytes: /verify/suite.json serves SUITE_TEXT exactly; /verify's
 *      digest matches those bytes; suite rows replay green in-process
 *   5b. The doors the GET-only suite cannot exercise: POST /positions + MCP
 *   6. Conneg spot check: curl-class JSON / agent-UA markdown / browser HTML at /
 *   7. RULED BRIDGES (batch-2 rollup, zero divergence): rates[] top-level in
 *      the Pricing Document; g2 top-level on the card; links.verify as a card
 *      link member; operationId on every OpenAPI route; rates[].operation ⊆
 *      OpenAPI operationIds; every rate row freeQuota or zero price
 *
...
{
  const corpora = {
    instruments: substrate.instruments,
    quotes: substrate.quotes,
    positions: substrate.seedPositions,
    postTradeDocuments: substrate.postTradeDocuments,
  }
  let bad = 0
  for (const [name, rows] of Object.entries(corpora)) {
    for (const rec of rows) {
      if (rec.example !== true) { fail(`${name}/${rec.id}: missing example: true`); bad++ }
      for (const labelField of ['name', 'title']) {
        if (labelField in rec && !String(rec[labelField]).startsWith('[demo]')) {
          fail(`${name}/${rec.id}: ${labelField} lacks the [demo] prefix`)
          bad++
        }
      }
      if ('symbol' in rec && !String(rec.symbol).startsWith('ZZ')) {
        fail(`${name}/${rec.id}: symbol ${rec.symbol} is not ZZ-prefixed synthetic`)
        bad++
      }
    }
  }
  if (bad === 0) pass(`${Object.values(corpora).flat().length} seed records labeled (example: true; [demo] names/titles; ZZ symbols)`)
}
...
console.log('5b. headless PMS door + MCP door')
{
  const created = await worker.fetch(
    new Request(`${ORIGIN}/positions`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ accountId: 'acct_demo_01', instrumentId: 'ins_0002', quantity: 40, costBasis: 668.0 }),
    }),
  )
  const createdBody = await created.json()
  if (created.status !== 201 || createdBody.type !== 'OK' || createdBody.results[0].example !== true) {
    fail(`POST /positions (recordPosition): status ${created.status}, expected 201 OK with a labeled record`)
  } else pass('recordPosition answers 201 OK; the recorded record is labeled and discloses ephemeral retention')
  ...
  const call = await rpc('tools/call', { name: 'listInstruments', arguments: { assetClass: 'bond' } })
  const nTools = tools.result?.tools?.length ?? 0
  const callOk = call.result?.content?.[0]?.text?.includes('"OK"')
  if (nTools !== 7) fail(`MCP tools/list: ${nTools} tools, expected 7`)
  else if (!callOk) fail('MCP tools/call listInstruments did not answer an OK envelope')
  else pass('MCP door: 7 tools listed (keyless — only the anon-sandbox rung is mounted); tools/call answers the same typed envelopes as HTTP')
}
...
// ── 7. ruled bridges (batch-2 rollup — zero divergence tolerated) ───────────
console.log('7. ruled bridge placements')
{
  const card = await (await worker.fetch(new Request(`${ORIGIN}/.well-known/agents.json`))).json()
  if (!card.g2 || !card.g2.icp || !Array.isArray(card.g2.personas)) fail('card lacks top-level g2 (icp + personas)')
  else pass('g2 rides top-level on the card')
  if (card.links?.verify !== `${ORIGIN}/verify`) fail(`card links.verify is ${card.links?.verify}, expected ${ORIGIN}/verify`)
  else pass('links.verify rides as a card link member')

  const pricing = await (await worker.fetch(new Request(`${ORIGIN}/pricing`, { headers: { accept: '*/*', 'user-agent': 'curl/8.6.0' } }))).json()
  if (!Array.isArray(pricing.rates) || pricing.rates.length === 0) fail('Pricing Document lacks top-level rates[]')
  else {
    let bad = 0
    for (const row of pricing.rates) {
      if (row.price !== 0 && row.freeQuota === undefined) { fail(`rate row ${row.operation}: neither zero price nor freeQuota`); bad++ }
    }
    if (bad === 0) pass(`rates[] top-level in the Pricing Document (${pricing.rates.length} rows, all priced from zero)`)
  }
  // faces coherence: the md face embeds the same bridged document
  const pricingMd = await (await worker.fetch(new Request(`${ORIGIN}/pricing.md`))).text()
  if (!pricingMd.includes('"rates"')) fail('/pricing.md face lacks the bridged rates[] — faces diverged')
  else pass('/pricing.md face carries the same bridged rates[]')
  ...
  for (const [path, methods] of Object.entries(openapi.paths)) {
    for (const [method, op] of Object.entries(methods)) {
      if (!op.operationId) { fail(`openapi ${method.toUpperCase()} ${path}: no operationId`); missing++ }
      else ids.add(op.operationId)
    }
  }
  if (missing === 0) pass(`operationId on every route (${ids.size} distinct ids)`)
  let outside = 0
  for (const row of pricing.rates || []) {
    if (!ids.has(row.operation)) { fail(`rates[].operation ${row.operation} ∉ OpenAPI operationIds`); outside++ }
  }
  if (outside === 0 && Array.isArray(pricing.rates)) pass('rates[].operation ⊆ OpenAPI operationIds')
}

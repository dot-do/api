 ...
 * axp-ext-rates-g2@0.2.0 native — the survey floor), unwrapped. The four
 * extension members (pricing `rates[]`, card `links.verify`, card `g2`,
 * per-route operationIds) are NATIVE generator inputs — declared in
 * ./manifest.ts, validated fail-closed at `defineSiteManifest`, emitted at
 * the ruled placements by the generator itself. No hand-patched documents,
 * no overridden routes, no bridges: one manifest in, the quartet out.
 */

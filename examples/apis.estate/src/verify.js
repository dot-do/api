 ...
 * undeclared until digest-pinned (batch watch list) — declaring it arms the
 * pinned spec's check-published-test-suite, which the deployed verifier
 * cannot yet judge (the api.lawyer precedent). The suite itself ships at
 * /verify/suite.json, and the card carries links.verify natively
 * (manifest verifyUrl — axp-ext/rates-g2 §3).
 */
...
export function buildSuite(manifest) {
  ...
  const checks = [
    ...
    {
      id: 'escrow-blocked',
      probe: `GET ${o}${probes.keyless.url}?scope=escrow`,
      expect: { status: 403, 'body.type': 'BLOCKED' },
    },
    {
      id: 'transaction-file-mint',
      probe: `POST ${o}/transaction-files`,
      expect: { status: 200, 'body.type': 'OK' },
    },
  ]
  return {
    suite: 'apis.estate public-contract checks',
...
export function buildVerifyDoc(manifest) {
  ...
  return {
    ...
    name: 'Run our tests',
    description:
      'The public-contract checks for apis.estate. Every check is runnable by anyone, keyless, against the live origin — a claim with a published passing test is proven; anything less stays out of the copy.',
...
export function buildVerifyMd(manifest) {
  ...
  const lines = [
    '# Run our tests',
    '',
    'The public-contract checks for apis.estate — runnable by anyone, keyless, against the live origin.',
    '',

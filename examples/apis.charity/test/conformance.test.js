 ...
 * (§9.1: local conformance green at the pinned digest, --expect-digest
 ...
 * digest below is the ratification digest — if the vendored bytes ever
 * drift, `assertConforms` refuses before a single probe fires. The deployed
 * verifier at https://api.qa runs the SAME digest-locked requirement
 * implementations, so this gate green and a hosted verdict cannot diverge
 * by construction.
 *
 * Probes dispatch to the worker IN MEMORY (no socket) at the canonical
 * origin — including through ./src/bridge.js, so the four ruled extension
 * placements are proven ADDITIVE: the bridged documents pass the pinned
...
describe('AXP conformance — pinned apis-ax-axp@2.6.0 (fail-closed digest gate)', () => {
  it(`every pinned requirement passes at digest ${PINNED_DIGEST.slice(0, 12)}…`, async () => {
    await assertConforms(worker, { spec, expectedDigest: PINNED_DIGEST }, { baseOrigin: 'https://apis.charity' })
  }, 30_000)

  it('the advisory grade is a full pass with an empty failing set', async () => {
    await assertGradeAtLeast(worker, 'A+', { baseOrigin: 'https://apis.charity' })
    const report = await grade(worker, { baseOrigin: 'https://apis.charity' })

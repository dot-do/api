 ...
 * apis.salon origin, so the card's absolute same-origin cross-links hold
 * exactly as they will live.
 *
 * DISCLOSURE (batch watch list): the vendored axp-faces@0.3.0 ships no
 * describeConformance — the probe ladder runs in-process via the verifier's
 * own gradePinned/assertConforms at the pinned digest instead, which is the
 * stricter instrument (the deployed api.qa code path itself).
 */
...
describe("AXP conformance — pinned apis-ax-axp@2.6.0 (fail-closed digest gate)", () => {
  it(`every pinned requirement passes at digest ${PINNED_DIGEST.slice(0, 12)}…`, async () => {
    await assertConforms(worker, { spec, expectedDigest: PINNED_DIGEST }, { baseOrigin: "https://apis.salon" });
  }, 30_000);
});

 ...
 * verifier does not implement — a declaration today fails closed on a check
 * the verifier cannot produce (the api.lawyer precedent). The suite itself
 * ships at /verify/suite.json, and the card carries links.verify natively
...
export function buildSuite(manifest) {
  ...
  const checks = [
    ...
    {
      id: 'catalog-mint',
      probe: `POST ${o}/catalogs`,
      expect: { status: 200, 'body.type': 'OK' },
    },
  ]
  return {
    suite: 'apis.education public-contract checks',
...
export function buildVerifyDoc(manifest) {
  ...
  return {
    ...
    name: 'Run our tests',
    description:
      'The public-contract checks for apis.education. Every check is runnable by anyone, keyless, against the live origin — a claim with a published passing test is proven; anything less stays out of the copy.',
...
export function buildVerifyMd(manifest) {
  ...
  const lines = [
    '# Run our tests',
    '',
    'The public-contract checks for apis.education — runnable by anyone, keyless, against the live origin.',
    '',

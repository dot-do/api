 ...
 * agent-referred HUMAN traffic on a developer face = missing counterpart
 * brand in this cell).
 */
...
const BASE = Object.freeze({
  substrate: SUBSTRATE,
  projection: projection.brand, // apis.supply — registered projection; face pending provisioning (projection.js brandStatus)
...
function emit(seam, payload) {
  ...
  if (globalThis.__SEAM_SILENT !== true) console.log(JSON.stringify({ seam, ts: new Date().toISOString(), ...payload }));
}
 ...
 *  boundary that exists is the 402 OFFER being SERVED (a stub, no settlement).
 *  Never a fake charge. */
...
/** Signup/traffic events — identity class + referral source (§9.3). */

...
export function buildSuite(manifest) {
  ...
  const checks = [
    ...
    {
      id: 'project-mint',
      probe: `POST ${o}/projects`,
      expect: { status: 200, 'body.type': 'OK' },
    },
  ]
  return {
    suite: 'apis.engineering public-contract checks',
...
export function buildVerifyDoc(manifest) {
  ...
  return {
    ...
    name: 'Run our tests',
    description:
      'The public-contract checks for apis.engineering. Every check is runnable by anyone, keyless, against the live origin — a claim with a published passing test is proven; anything less stays out of the copy.',
...
export function buildVerifyMd(manifest) {
  ...
  const lines = [
    '# Run our tests',
    '',
    'The public-contract checks for apis.engineering — runnable by anyone, keyless, against the live origin.',
    '',

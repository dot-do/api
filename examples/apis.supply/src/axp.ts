 ...
 * never patched into the vendored files). Placements are RULED (batch-2
 * watch list): rates[] top-level in the Pricing Document, g2 top-level on
 * the card, links.verify as a card link member, operationId on every route.
 *
...
const pricingFaces = {
  json: pricingDoc,
  md: mdOfJson('apis.supply — pricing (rate card)', pricingDoc),
  html: htmlOfJson('apis.supply — pricing (rate card)', pricingDoc),
}

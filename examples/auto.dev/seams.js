 ...
 *
 * The §9.3 standing diagnostic runs on exactly these events: agent-referred
 * HUMAN traffic arriving on this developer brand is the vin.company signal
 * (the spec's own exemplar came from this row) — identityClass × referral
 * source below is what the harness reads.
 */

const SUBSTRATE = "automotive";
const PROJECTION = "auto.dev";
const MOTION = "B2D";
const PATTERN = "402-metered-per-call"; // experiment registration, projection.json
...
/** Money-event seam (settlement-worker ledger pattern) — STUB on this face:
 *  emitted only from the 402 boundary with settled:false; no settlement is
 *  wired on this examples worker (the production rail settles at the Drivly
 *  stack — that ledger is that stack's, never claimed here). */

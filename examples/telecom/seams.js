...
const BASE = Object.freeze({
  substrate: "telecom",
  projection: "telecom.org.ai", // row-key placeholder — true-zero GAP row; re-tag at brand attach (#16)
  motion: "B2A",
  pattern: "per-outcome",
});
 ...
 *  the §9.3 missing-counterpart-brand diagnostic's raw feed. This cell's ICP
 *  includes IT-ops seats and reseller operators who do not speak API — and the
 *  row holds NO human-vocabulary name at all (true-zero), so agent-referred
 *  HUMAN traffic here is a direct counterpart-brand-proposal trigger against
 *  the register row (named per #3, never vocabulary patches here). */

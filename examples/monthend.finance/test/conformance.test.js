 ...
 * canonical origin, so the card's absolute same-origin cross-links hold
...
describe("AXP conformance — pinned apis-ax-axp@2.6.0 (fail-closed digest gate)", () => {
  it(`every pinned requirement passes at digest ${PINNED_DIGEST.slice(0, 12)}…`, async () => {
    await assertConforms(worker, { spec, expectedDigest: PINNED_DIGEST }, { baseOrigin: "https://monthend.finance" });
  }, 30_000);
});
